...
class BibliotecaComic(models.Model):
    ...
    soci_ids = fields.Many2many('biblioteca.soci', string='Socis')
...
class BibliotecaSoci(models.Model):
    ...
    _description = 'Soci de la biblioteca'
...
class BibliotecaComicEjemplar(models.Model):
    ...
    _description = 'Ejemplar de un cómic'
    ...
    codigo = fields.Char('Código del Ejemplar', required=True, unique=True)
    estado = fields.Selection(
        [('disponible', 'Disponible'), 
        ('prestado', 'Prestado'), 
        ('dañado', 'Dañado')],
    ...
    fecha_prestamo = fields.Date('Fecha de Préstamo', readonly=True)
    fecha_devolucion = fields.Date('Fecha de Devolución', readonly=True)

    def prestar(self):
        """Cambia el estado a 'Prestado'."""
        for record in self:
            if record.estado != 'disponible':
                raise ValidationError('Solo se pueden prestar ejemplares disponibles.')
    ...
    def devolver(self):
        ...
        for record in self:
            if record.estado != 'prestado':
                raise ValidationError('Solo se pueden devolver ejemplares prestados.')
